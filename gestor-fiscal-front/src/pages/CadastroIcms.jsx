...
export default class CadastroIcms extends Component {
  ...
  componentDidMount() {
    axios.get(`http://localhost:8080/api/icms`)
      .then(res => {
        const data = res.data;
        this.setState({ data });
  ...
  cadastraItem = () => {
    if (this.state.estado === '' || this.state.sigla === 0 || this.state.icms === '') {
      ...
      axios.post(`http://localhost:8080/api/icms`, {
  ...
  alteraItem = () => {
    axios.put(`http://localhost:8080/api/icms`, {
      id: this.state.id,
      sigla: this.state.sigla,
      estado: this.state.estado,
      icms: this.state.icms
  ...
  deletaIcms = (id) => {
    axios.delete(`http://localhost:8080/api/icms/` + id)
      .then(res => {
        this.atualizaTabela();
      }
      )
